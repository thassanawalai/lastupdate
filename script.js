  ...
  displayFinalResult() {
    ...
    let html = `
      <div class="result-summary">
        <h2>สรุปผลการประเมินสุขภาพจิต</h2>
        <div class="result-section">
          <h3>ข้อมูลส่วนตัว</h3>
          <p><b>ชื่อ:</b> ${appState.formData.personalInfo.nickname || 'ไม่ได้ระบุ'}</p>
          <p><b>อายุ:</b> ${appState.formData.personalInfo.age || 'ไม่ได้ระบุ'}</p>
        </div>
        <div class="result-section">
          <h3>ผลการประเมิน ST-5</h3>
          <p><b>คะแนน:</b> ${appState.formData.results.st5?.score || 0}</p>
          <p>${appState.formData.results.st5?.message || ''}</p>
        </div>
        <div class="result-section">
          <h3>ผลการประเมิน 2Q</h3>
          <p>${appState.formData.results.twoQ?.message || ''}</p>
        </div>
        <div class="result-section">
          <h3>ผลการประเมิน 9Q</h3>
          <p><b>คะแนน:</b> ${appState.formData.results.nineQ?.score || 0}</p>
          <p>${appState.formData.results.nineQ?.message || ''}</p>
        </div>
        <div class="result-section">
          <h3>ผลการประเมิน 8Q</h3>
          <p><b>คะแนน:</b> ${appState.formData.results.eightQ?.score || 0}</p>
          <p>${appState.formData.results.eightQ?.message || ''}</p>
        </div>
      </div>
    `;
    
    finalResultElement.innerHTML = html;
